   ...
   */
   ...
   public void moveEntity( Entity entity, Point pos)
   {
      ...
      if (this.withinBounds( pos) && !pos.equals(oldPos) && !isOccupied(pos)){
         ...
         if (!ent.isEmpty() && ent.get().getClass() == AGrade.class && !((Player) entity).getHasA()) {
            ((Player) entity).swapA();
            this.setOccupancyCell( oldPos, null);
            this.removeEntityAt( pos);
            this.setOccupancyCell( pos, entity);
            ...
            List<Point> neighbors = CARDINAL_NEIGHBORS.apply(oldPos)
                    .filter(n -> !ent.isEmpty() && ent.get().getClass() == GradeBook.class)
                    .collect(Collectors.toList());
            if(neighbors.size() != 0){
               ((Player) entity).swapA();
            }
         }
         else if(!ent.isEmpty() && (ent.get().getClass() == FGrade.class || ent.get().getClass() == Sickness.class)) {
            //Game Over
